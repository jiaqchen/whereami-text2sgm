...
save_files_dir = './distributions/subgraphing_no_attrib/'
...
for text_graph in text_graphs:
    if len(text_graph.nodes) > 0:
        num_good_text += 1
print(f'length of scene graphs: {len(scene_graphs)} and num good: {num_good}')
print(f'length of text graphs: {len(text_graphs)} and num good: {num_good_text}')
...
print(f'len of zipped graphs: {len(list(zip(scene_graphs, text_graphs)))}')
